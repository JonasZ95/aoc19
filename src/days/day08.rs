...
impl<'a> Layer<'a> {
    ...
    fn digits_iter(&self) -> impl Iterator<Item = u8> + '_ {
        (0..self.img.layer_size())
            .map(move |i| self.img.data[self.ix + i])
    }
...
impl Image {
    ...
    fn layers_iter(&self) -> impl DoubleEndedIterator<Item=Layer<'_>> + '_ {
        (0..self.layers_len())
            .map(move 
                |ix| self.layer(ix).unwrap())
    }

    fn lowest_layer(&self) -> usize {
        let min_layer = self.layers_iter()
            .min_by_key(|l| l.digits_iter().filter(|&d| d == 0).count())
            .unwrap();

        let ones = min_layer.digits_iter()
            .filter(|&d| d == 1)
            .count();

        let twos = min_layer.digits_iter()
            .filter(|&d| d == 2)
            .count();
    ...
    fn draw_image(&self) -> Vec<Vec<u8>> {
        self.layers_iter().rev().fold(vec![vec![0; self.width]; self.height],
            |mut img, layer| {
                for (x, y, pixel) in layer.rows_iter().enumerate()
