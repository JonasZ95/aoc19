...
pub fn settings_perm(from: u8) -> impl Iterator<Item = [u8; 5]> + 'static {
    (from..from + 5).permutations(5).map(|s| {
        let mut out = [0; 5];
...
pub fn exec_amps(data: &Data, settings: [u8; 5], input: isize) -> AocResult<isize> {
    settings.iter().try_fold(input, |signal, &s| {
        Context::from_data(data.clone(), &[s as isize, signal]).exec()
    })
}

fn find_max_signal2(data: Data, input: isize) -> AocResult<isize> {
    Ok(
        convert(settings_perm(5).map(|s| exec_amps2(&data, s, input)))
            .max()?
            .unwrap(),
    )
}
...
pub fn exec_amps2(data: &Data, settings: [u8; 5], input: isize) -> AocResult<isize> {
    let mut ctxs: Vec<Context> = settings
        .iter()
        .map(|&s| Context::from_data(data.clone(), &[s as isize]))
        .collect();
...
fn find_max_signal(data: Data, input: isize) -> AocResult<isize> {
    Ok(
        convert(settings_perm(0).map(|s| exec_amps(&data, s, input)))
            .max()?
            .unwrap(),
    )
}
...
mod tests {
    ...
    fn part1_test() -> AocResult<()> {
        ...
        let data: Data =
            "3,23,3,24,1002,24,10,24,1002,23,-1,23,101,5,23,23,1,24,23,23,4,23,99,0,0".parse()?;
